 ...
 *
 * 为什么需要它:文档最容易静默过期——README 说"只监听 127.0.0.1"、
 * HANDOFF 停在旧版本号,这类不一致人工很难发现,而且一旦发现就是"读者被误导"。
 * 本脚本把"文档里的事实"与"代码里的事实"对照,不一致就报错退出(exit 1)。
 *
 * 检查项:
 *   1. 文档提到的 CLI 子命令是否真实存在
 *   2. 版本号三方一致(GATEWAY_VERSION / package.json / UPSTREAM.md)
 *   3. README 提到的 OSG_* 环境变量是否真的被读取
...
// ---------- 4. FALLBACK_CODEX_VERSION 与 UPSTREAM.md 对齐 ----------
...
if (problems.length === 0) {
  process.stdout.write(
    `DOC_FRESHNESS: OK\n` +
      ...
      `  已校验:命令存在性 / 版本三方一致 / 环境变量真实读取 / fallback 版本对齐 / 引用路径存在\n` +
